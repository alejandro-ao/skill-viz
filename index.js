...
.sidebar-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.icon-btn{border:1px solid var(--line);background:var(--surface);color:var(--text);border-radius:4px;cursor:pointer;box-shadow:var(--shadow);width:36px;height:36px;flex:0 0 auto}.icon-btn:hover{background:var(--surface-2)}
...
function syncThemeButtons(){themeToggles.forEach(btn=>btn.textContent=document.documentElement.dataset.theme === 'dark' ? '☾' : '☼')}
syncThemeButtons();
